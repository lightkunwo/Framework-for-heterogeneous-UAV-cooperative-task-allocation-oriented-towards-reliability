...
def finalize_simulation_outputs(simulator, realtime_plot, replan2ga, start_time, position, distance,
    ...
    with open(stats_path, 'w', encoding='utf-8') as f:
        ...
        if self.final_unfinished_summary:
            ...
            f.write("Unfinished Task List (target_id, task_type):\n")
            for task in ms.get('unfinished_tasks', []):
                f.write(f"  {task}\n")
            if not ms.get('unfinished_tasks'):
                f.write("  None\n")
            f.write("\n")
            f.write("Blocked Task List (target_id, task_type):\n")
            for task in ms.get('blocked_tasks', []):
                f.write(f"  {task}\n")
            if not ms.get('blocked_tasks'):
                f.write("  None\n")
            f.write("\n")
        ...
        if self.subsystem_events:
            ...
            for i, event in enumerate(self.subsystem_events, 1):
                ...
                if event.get('failure_reason'):
                    f.write(f"  Failure Reason: {event.get('failure_reason')}\n")
                    f.write(f"  Failure Details: {event.get('failure_details', {})}\n\n")
...
class DynamicSEADMissionSimulator(object):
    ...
    def _summarize_unfinished_tasks(self, completed_tasks_list, alive_state):
        ...
        summary = {
            ...
            'unfinished_tasks': sorted([(int(t[0]), int(t[1])) for t in unfinished], key=lambda x: (x[0], x[1])),
            'blocked_tasks': [],
        }
        ...
        for task in unfinished:
            ...
            if allowed_types & alive_types:
                ...
                summary['blocked_tasks'].append((tid, ttype))
        ...
        if len(self.subsystem_events) == 0:
            ...
            for i, ev in enumerate(self.subsystem_events, 1):
                ...
                if members:
                    ...
                    detail = ev.get('failure_details', {})
                    print(f'Event {i}: UAV{failed} failed at t={t:.2f}s -> Subsystem selection failed ({reason})')
                    if detail:
            ...
            if surveillance[0] == 100:
                ...
                if self.enable_subsystem:
                    ...
                    if selected_uavs:
                        ...
                        # Final selection succeeded; clear failure markers.
                        subsystem_info["selection_success"] = True
                        subsystem_info["failure_reason"] = None
                        subsystem_info["failure_details"] = {}
                    else:
                        if not subsystem_info:
                            subsystem_info = {}
                        subsystem_info.setdefault("reliability", 0.0)
                        subsystem_info.setdefault("type_distribution", {})
                        subsystem_info.setdefault("selection_success", False)
                        subsystem_info.setdefault("failure_reason", "unknown")
                        subsystem_info.setdefault("failure_details", {})
                    ...
                    self.subsystem_events.append({
                        ...
                        'failure_reason': subsystem_info.get('failure_reason'),
                        'failure_details': subsystem_info.get('failure_details', {}),
                        'decision': decision,
                        'p_rebuild': p_rebuild,
                        'migrate_mode': migrate_mode,
                        'migrate_ratio': migrate_ratio,
                    ...
                    if selected_uavs:
                        ...
                        reason = subsystem_info.get('failure_reason', 'unknown')
                        detail = subsystem_info.get('failure_details', {})
                        print(f"\n  Subsystem selection failed ({reason}), all UAVs continue current tasks")
                        if detail:
                            print(f"  detail={detail}")
                        print("")
                else:
...
if __name__ == '__main__':
    ...
    dynamic_SEAD_mission = DynamicSEADMissionSimulator(
        ...
        reliability_alpha_dict={1: 0.00002, 2: 0.00003, 3: 0.00009},
