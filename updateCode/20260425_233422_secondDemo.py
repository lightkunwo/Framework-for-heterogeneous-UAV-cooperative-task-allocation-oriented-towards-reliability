...
class UAV(object):
    ...
    def get_reliability(self, current_time):
    ...
    def check_failure(self, current_time, threshold=0.98):
        if self.is_failed:
            return True
        current_reliability = self.get_reliability(current_time)
...
class DynamicSEADMissionSimulator(object):
    ...
    def __init__(self, targets_sites, uav_id, uav_type, cruise_speed, turning_radii, initial_states, base_locations,
        ...
        # Actor-Critic (PyTorch) for state->decision (internal/rebuild)
        self.use_actor_critic = True
        self.ac_state_dim = 10
        self.ac_hidden_dim = 64
        self.ac_gamma = 0.95
        self.ac_actor_lr = 0.001
        self.ac_critic_lr = 0.002
        self.use_torch_actor_critic = bool(self.use_actor_critic and TORCH_AVAILABLE)
        self.ac_device = None
        self.ac_actor_alpha = None
        self.ac_actor_beta = None
        self.ac_critic = None
        self.ac_actor_optimizer = None
        self.ac_critic_optimizer = None
        if self.use_torch_actor_critic:
            self.ac_device = torch.device("cpu")
            self.ac_actor_alpha = PolicyNet(self.ac_state_dim, self.ac_hidden_dim, 2).to(self.ac_device)
            self.ac_actor_beta = PolicyNet(self.ac_state_dim, self.ac_hidden_dim, 2).to(self.ac_device)
            self.ac_critic = ValueNet(self.ac_state_dim, self.ac_hidden_dim).to(self.ac_device)
            self.ac_actor_optimizer = torch.optim.Adam(
                list(self.ac_actor_alpha.parameters()) + list(self.ac_actor_beta.parameters()),
                lr=self.ac_actor_lr
            )
            self.ac_critic_optimizer = torch.optim.Adam(self.ac_critic.parameters(), lr=self.ac_critic_lr)
        self.ac_update_steps = 0
    ...
    def _record_policy_point(self, event_time, subsystem_id, trigger, decision,
        ...
        """Record alpha/beta policy values for plotting and post-run analysis."""
        try:
    ...
    def _build_subsystem_state(self, subsystem_uav_ids, reliability_history, alive_state, all_completed_tasks,
        ...
        type_code, type_presence_count, type_counts = self._encode_type_code(alive_members, alive_state)
        unfinished_tasks = self._extract_unfinished_tasks(all_completed_tasks)
        n_remain = len(unfinished_tasks)
        d_spread = self._compute_task_spread(unfinished_tasks)
        d_uav_task, d_uav_task_std, d_uav_dispersion = self._compute_uav_position_metrics(
            alive_members, uav_positions if uav_positions is not None else {}, unfinished_tasks
        )
    ...
    def _update_actor_critic(self, prev_state, next_state, decision, migrate_mode, terminal=False):
        if not self.use_torch_actor_critic:
            return
        s = self._state_to_vector(prev_state)
        s_next = self._state_to_vector(next_state)
        action_alpha = 1 if decision == "rebuild" else 0
        action_beta = 1 if migrate_mode == "internal" else 0

        state_t = torch.tensor(s[np.newaxis, :], dtype=torch.float32, device=self.ac_device)
        next_state_t = torch.tensor(s_next[np.newaxis, :], dtype=torch.float32, device=self.ac_device)
        action_alpha_t = torch.tensor([[action_alpha]], dtype=torch.long, device=self.ac_device)
        action_beta_t = torch.tensor([[action_beta]], dtype=torch.long, device=self.ac_device)

        reward = self._compute_policy_reward(prev_state, next_state, decision)
        reward_t = torch.tensor([[reward]], dtype=torch.float32, device=self.ac_device)
        with torch.no_grad():
            next_v = torch.zeros((1, 1), dtype=torch.float32, device=self.ac_device)
            if not terminal:
                next_v = self.ac_critic(next_state_t)
            td_target = reward_t + self.ac_gamma * next_v

        value_s = self.ac_critic(state_t)
        td_delta = td_target - value_s

        alpha_probs = self.ac_actor_alpha(state_t)
        beta_probs = self.ac_actor_beta(state_t)
        ...
        self.ac_actor_optimizer.zero_grad()
        self.ac_critic_optimizer.zero_grad()
        actor_loss.backward()
        critic_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.ac_actor_alpha.parameters(), max_norm=5.0)
        torch.nn.utils.clip_grad_norm_(self.ac_actor_beta.parameters(), max_norm=5.0)
        torch.nn.utils.clip_grad_norm_(self.ac_critic.parameters(), max_norm=5.0)
        self.ac_actor_optimizer.step()
        self.ac_critic_optimizer.step()
        ...
        self.ac_update_steps += 1
        if self.ac_update_steps % 25 == 0:
            print(
                f"[AC] steps={self.ac_update_steps}, reward={reward:.3f}, td={td_error:.3f}, "
                f"alpha={prob_rebuild:.3f}, beta={prob_internal:.3f}"
            )

    def _decide_replan_action(self, subsystem_state):
        n = max(1, int(subsystem_state.get("N", 0)))
        r_mean = float(subsystem_state.get("R_mean", 0.0))
        type_presence_count = self._decode_type_presence(subsystem_state)
        ...
        remain_pressure = float(np.clip(n_remain / max(1.0, len(self.targets_sites) * 3.0), 0.0, 1.0))
        spread_pressure = float(np.clip(d_spread / max(1e-6, self.map_diag), 0.0, 1.0))
        uav_task_pressure = float(np.clip(d_uav_task / max(1e-6, self.map_diag), 0.0, 1.0))
        uav_task_imbalance = float(np.clip(d_uav_task_std / max(1e-6, 0.5 * self.map_diag), 0.0, 1.0))
        formation_pressure = float(np.clip(d_uav_dispersion / max(1e-6, self.map_diag), 0.0, 1.0))
        idle_relief = float(np.clip(n_idle / n, 0.0, 1.0))

        if self.use_torch_actor_critic:
            state_vec = self._state_to_vector(subsystem_state)
            alpha_rebuild = self._actor_rebuild_prob(state_vec)
            beta_internal = self._actor_internal_migrate_prob(state_vec)
        else:
            # alpha: subsystem-rebuild probability
            score_alpha = (
                0.10
                + 0.28 * rel_risk
                + 0.20 * type_risk
                + 0.12 * remain_pressure
                + 0.08 * spread_pressure
                + 0.12 * uav_task_pressure
                + 0.05 * uav_task_imbalance
                + 0.05 * formation_pressure
                - 0.10 * idle_relief
            )
            alpha_rebuild = float(np.clip(score_alpha, 0.05, 0.95))
            # beta: internal-migration probability (independent from alpha)
            score_beta = (
                0.55
                + 0.20 * idle_relief
                + 0.10 * (1.0 - remain_pressure)
                + 0.05 * (1.0 - spread_pressure)
                - 0.20 * type_risk
                - 0.10 * uav_task_pressure
                - 0.05 * formation_pressure
            )
        ...
        # alpha -> rebuild/internal-replan decision
        # beta  -> internal/external migration mode
        ...
        migrate_ratio = self.internal_migrate_ratio if migrate_mode == "internal" else self.external_migrate_ratio

        return {
            "alpha_rebuild": alpha_rebuild,
            "p_rebuild": alpha_rebuild,  # backward compatibility
            "decision": decision,
    ...
    def task_allocation_process_replan(self, ga2control_queue, control2ga_queue, output_interval=0.5):
        ...
        global_elite_bank = []
        local_elite_bank = {}
        prev_transfer_distance = {}
        last_migrate_epoch = None
        task_type_to_uav_type = {
            1: {1, 2},  # reconnaissance task
            2: {2, 3},  # attack task
            3: {1, 2},  # verification task
        }

        def _clone_chromosome(chrom):
            return [list(row) for row in chrom]

        def _extract_elites(pop, k=8):
            if not pop:
                return []
            ...
            for ind in ranked[:k]:
                try:
                    if ind.chromosome and len(ind.chromosome) == 5:
                        elites.append(_clone_chromosome(ind.chromosome))
                except Exception:
                    continue
            return elites

        def _build_tasktype_uav_ids(uav_info):
            mapping = {1: [], 2: [], 3: []}
            uav_ids = list(getattr(uav_info, "uav_id", []))
            uav_types = list(getattr(uav_info, "uav_type", []))
            for uid, utype in zip(uav_ids, uav_types):
                for task_kind, allowed_types in task_type_to_uav_type.items():
        ...
        def _pick_focus_uav(candidate_uavs, target_chrom):
        ...
        def _safe_centers_for_uav(chrom_list, length, focus_uav):
            if not chrom_list:
                return None
            heading_values = []
            for chrom in chrom_list:
        ...
        def _adaptive_radius(center_distance, key):
            prev_dist = prev_transfer_distance.get(key, None)
            if prev_dist is None:
                ...
                radius = 1.0
            prev_transfer_distance[key] = center_distance
            if not np.isfinite(radius) or radius <= 0:
                radius = 1.0
            return float(np.clip(radius, 0.1, 2.5))

        def _center_map_source_to_target(source_chrom, target_chrom, radius,
                                         source_center, target_center,
                                         task_uav_ids, fallback_uav_ids, focus_uav):
            new_chrom = _clone_chromosome(target_chrom)
            ...
            for g in range(length):
                ...
                if int(focus_uav) in allowed_uavs:
                    ...
                    new_chrom[3][g] = _nearest_valid(mapped_uav, allowed_uavs, fallback_uav_ids)
                ...
                if mapped_heading < 0:
                    mapped_heading = 0
                if mapped_heading > 35:
                    mapped_heading = 35
                new_chrom[4][g] = mapped_heading
                changed += 1

            # If target chromosome has no gene of focus_uav, force one compatible gene as migration carrier.
            if changed == 0:
                for g in range(length):
                    task_kind = int(new_chrom[2][g]) if g < len(new_chrom[2]) else 1
                    allowed_uavs = task_uav_ids.get(task_kind, [])
        ...
        def _inject_elites(pop, elites, ratio, scope_key, migrate_mode, uav_info):
            ...
            for i in range(min(inject_num, len(worst_indices))):
                ...
                source_chrom = _clone_chromosome(compatible_sources[i % len(compatible_sources)])
                focus_uav = _pick_focus_uav(fallback_uav_ids, target_chrom)
                if focus_uav is None:
                    continue
                source_center = _safe_centers_for_uav(compatible_sources, gene_len, focus_uav)
                if source_center is None:
                    source_center = _safe_centers(compatible_sources, gene_len)
                ...
                for ind in pop:
                    try:
                        ...
                        if chrom and len(chrom) == 5 and len(chrom[0]) == gene_len:
                            target_pool.append(_clone_chromosome(chrom))
                    except Exception:
                        continue
                target_center = _safe_centers_for_uav(target_pool, gene_len, focus_uav)
                if target_center is None:
                    target_center = _safe_centers(target_pool, gene_len)

                if source_center is None or target_center is None:
                    pop[target_idx] = mission.Chromosome(source_chrom)
                    continue
                ...
                radius = _adaptive_radius(center_distance, radius_key)

                # HUAV-MKT-TPD style: mapped = r * (knowledge - source_center) + target_center
                migrated_chrom, migrated_genes = _center_map_source_to_target(
                    source_chrom, target_chrom, radius,
                    source_center, target_center,
                    task_uav_ids, fallback_uav_ids, focus_uav
                )
                if migrated_genes > 0:
                    pop[target_idx] = mission.Chromosome(migrated_chrom)
                    applied += 1
        ...
        while True:
            ...
            # Migration injection before each replanning cycle
            ...
            beta_internal = float(getattr(uavs, "beta_internal", 0.5))
            beta_internal = float(np.clip(beta_internal, 0.05, 0.95))
            beta_external = float(1.0 - beta_internal)
            current_epoch = int(getattr(uavs, "subsystem_epoch", -1))
            # Event-driven migration: inject once per subsystem epoch (not every GA cycle).
            allow_migration_this_round = (current_epoch != last_migrate_epoch)
            if population is not None and migrate_ratio > 0 and allow_migration_this_round:
                if migrate_mode == "external":
                    source_elites = global_elite_bank
                elif migrate_mode == "internal":
                    source_elites = local_elite_bank.get(scope_key, [])
                else:
                    # backward-compatible fallback
                    source_elites = local_elite_bank.get(scope_key, []) if np.random.rand() < beta_internal else global_elite_bank
                if source_elites:
                    population, applied, subsystem_id = _inject_elites(
                        population, source_elites, migrate_ratio, scope_key, migrate_mode, uavs
                    )
                    last_migrate_epoch = current_epoch
                    if applied > 0:
                        print(
                            f"[MIGRATE][{subsystem_id}] mode={migrate_mode}, beta={beta_internal:.3f}/{beta_external:.3f}, "
                            f"ratio={migrate_ratio:.2f}, injected={applied}"
                        )
                    else:
                        print(
                            f"[MIGRATE][{subsystem_id}] mode={migrate_mode}, beta={beta_internal:.3f}/{beta_external:.3f}, "
                            f"ratio={migrate_ratio:.2f}, injected=0"
                        )
            ...
            # Update migration banks after each cycle
            current_elites = _extract_elites(population, k=8)
            if current_elites:
                local_elite_bank[scope_key] = current_elites
                global_elite_bank = (current_elites + global_elite_bank)[:24]
    ...
    def main_process(self, uav, u2u_communication, gcs_init_solution_queue,
        ...
        while True:
            current_reliability = uav.get_reliability(time.time() - start_time)
            ...
            if uav.check_failure(time.time() - start_time, self.failure_threshold):
    ...
    def generate_time_flow_plot(self, position, x, y, targets_sites, UAVs, failures,
        ...
        for i in range(1, 7):
            ...
            if index < len(position[position.index(max(position, key=len))]):
                time_stamp = position[position.index(max(position, key=len))][index][2] - start_time
            else:
                time_stamp = position[position.index(max(position, key=len))][-1][2] - start_time
        ...
        for ax, snap_idx, time_stamp in zip(axes, snapshot_indices, time_points):
            ...
            for uav_idx in range(len(position)):
                if snap_idx < len(position[uav_idx]):
                    ...
                    if len(trajectory_x) > 1:
                        ...
                        ax.plot(trajectory_x, trajectory_y, line_style, linewidth=1.5, color=color_style[uav_idx],
                                alpha=0.7)

                    if trajectory_x and trajectory_y:
                        ax.plot(trajectory_x[-1], trajectory_y[-1], '^',
                                markersize=10, color=color_style[uav_idx],
                                markeredgecolor='black', markeredgewidth=1.5)
                        ax.text(trajectory_x[-1] + 100, trajectory_y[-1] - 150,
            ...
            for target_idx, tgt in enumerate(targets_sites):
                ...
                for t_type in [1, 2, 3]:
                    completed = [log for log in self.task_completion_log
                                 if log['target_id'] == target_idx + 1 and
                                 log['task_type'] == t_type and
                                 log['time'] <= time_stamp]
                    executing = [log for log in self.task_completion_log
                                 if log['target_id'] == target_idx + 1 and
                                 log['task_type'] == t_type and
    ...
    def start_simulation(self, realtime_plot=False, uav_failure=None):
        ...
        for u in range(uav_num):
            if reliability_history[u]:
                ...
                print(f"  UAV {self.uavs[0][u]} ({uav_type_name}, α={UAVs[u].reliability_alpha}): "
                      f"初始={reliability_history[u][0]:.4f}, 末期={reliability_history[u][-1]:.4f}, "
                      f"故障={'是' if failures[u] else '否'}")
        print()

        # 任务总结（task_route风格 + 子系统事件）
        ...
        plt.axis("equal")
        plt.plot([xx[0] for xx in self.uavs[4]], [xx[1] for xx in self.uavs[4]], 'k^', markerfacecolor='none',
                 markersize=8)
        ...
        for t in self.targets_sites:
            plt.text(t[0] + 100, t[1] + 100, f'Target {self.targets_sites.index(t) + 1}', font1)
        for info in UAVs:
        ...
        for i in range(uav_num):
            ...
            ax.plot([p[0] for p in position[i]], [p[1] for p in position[i]], '-',
                    linewidth=2.0, color=color_style[i], label=f'UAV {self.uavs[0][i]}')
        ...
        self.generate_time_flow_plot(position, x, y, self.targets_sites, UAVs, failures,
        ...
        for i in range(uav_num):
            if len(time_history[i]) > 1:
                ...
                line_style = '-' if self.uavs[0][i] in active_subsystem or not active_subsystem else '--'
                ax.plot(time_history[i], reliability_history[i], line_style, linewidth=2, markersize=4,
        ...
        if self.policy_history:
            ...
            # Mark key moments for easier interpretation of policy switching.
...
if __name__ == '__main__':
    ...
    # All UAVs depart from their base locations directly.
    initial_states = [[float(base[0]), float(base[1]), float(base[2])] for base in base_locations]

    print("=" * 70)
    print("=" * 70)
    print(f"UAV数量: {len(uav_id)}")
    print(f"目标数量: {len(targets_sites)}")
    print(f"总任务数: {len(targets_sites) * 3} (侦察+攻击+验证)")
    print("UAV初始位置: 全部从基地位置出发")
    print("=" * 70 + "\n")
    ...
    dynamic_SEAD_mission = DynamicSEADMissionSimulator(
        ...
        reliability_alpha_dict={1: 0.00002, 2: 0.00005, 3: 0.00018},
